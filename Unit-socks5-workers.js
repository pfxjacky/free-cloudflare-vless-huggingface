...
// 新增：安全的域名解析
async function safeDomainResolve(hostname) {
...
function socks5Parser(socks5) {
	...
	if (isIPv6) {
		hostname = hostname.replace(/^\[|\]$/g, ''); // 移除IPv6地址的方括号
	}
...
function getConfigLink(uuid, host, address, port, remarks, proxyip, protType, nat64) {
	...
	if (socks5Enable && parsedSocks5) {
		const socks5Param = `${parsedSocks5.username}:${parsedSocks5.password}@${parsedSocks5.hostname}:${parsedSocks5.port}`;
		pathParm = pathParm + `&SOCKS5=${encodeURIComponent(socks5Param)}`;
	}
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, channelResponseHeader, log, addressType,) {
	...
	async function connectAndWrite(address, port, socks = false) {
		// 修改：安全域名解析
		address = await safeDomainResolve(address);
	...
	async function retry() {
		...
		if (socks5Enable && parsedSocks5) {
			...
			tcpSocket.closed.catch(error => {
				console.log('retry socks5 tcpSocket closed error', error);
			}).finally(() => {
		...
		const tcpSocket = await connectAndWrite(finalTargetHost, finalTargetPort);

		log(`retry-direct connected to ${addressRemote}:${portRemote}`);
		tcpSocket.closed.catch(error => {
...
async function socks5Connect(ipType, remoteIp, remotePort, log) {
	...
	try {
		// 添加连接超时
		const connectPromise = connect({ hostname, port });
		...
		const sendSocksRequest = async () => {
			...
			switch (ipType) {
				...
				case 3:
					// IPv6地址处理
					const ipv6Parts = remoteIp.split(':');
					const ipv6Bytes = [];
					...
					break;
				default:
					throw new Error(`Invalid address type: ${ipType}`);
			}
		...
		log(`SOCKS5 proxy connection established: ${hostname}:${port} -> ${remoteIp}:${remotePort}`);
		return socket;
