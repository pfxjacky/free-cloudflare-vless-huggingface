...
async function connectViaSocks5(proxyConf, targetHost, targetPort) {
  ...
  const sock = connect({ hostname: proxyConf.host, port: proxyConf.port, secureTransport: "off" });
  // 获取写入器和读取器
  ...
  try {
    ...
    if (isIPv4(targetHost)) {
      ...
      // treat all non-IPv4 hosts as domain name; let proxy resolve
      const enc = new TextEncoder();
