...
# Complete version with robust port detection
...
install_dependencies() {
    ...
    if command -v apt-get >/dev/null 2>&1; then
        apt-get update -y
        apt-get install -y python3 python3-pip curl net-tools openssl ufw iproute2 netcat-openbsd lsof
    elif command -v yum >/dev/null 2>&1; then
        yum update -y
        yum install -y python3 python3-pip curl net-tools openssl firewalld iproute nc lsof
    elif command -v dnf >/dev/null 2>&1; then
        dnf update -y
        dnf install -y python3 python3-pip curl net-tools openssl firewalld iproute nc lsof
    else
...
# Robust service verification function - key improvement here!
start_and_test_service() {
    ...
    # Robust verification with multiple attempts and methods
    local verification_passed=false
    local max_attempts=8
    
    for attempt in $(seq 1 $max_attempts); do
        info "Service verification attempt $attempt/$max_attempts..."
        
        # Method 1: Check service logs for "ready" message
        if journalctl -u "$SERVICE_NAME" --no-pager -n 20 | grep -q "SOCKS5 proxy server ready"; then
            verification_passed=true
            info "✓ Service ready message found in logs"
            break
        fi
        
        # Method 2: Check if process is listening with lsof
        if command -v lsof >/dev/null 2>&1; then
            if lsof -i ":$SOCKS5_PORT" 2>/dev/null | grep -q python; then
                verification_passed=true
                info "✓ Python process listening on port $SOCKS5_PORT (lsof)"
                break
            fi
        fi
        
        # Method 3: Check with ss/netstat
        if ss -tuln 2>/dev/null | grep -E "(0\.0\.0\.0:$SOCKS5_PORT|:::$SOCKS5_PORT|\[::\]:$SOCKS5_PORT)" >/dev/null 2>&1; then
            verification_passed=true
            info "✓ Port $SOCKS5_PORT listening (ss command)"
            break
        fi
        
        if netstat -tuln 2>/dev/null | grep -E "(0\.0\.0\.0:$SOCKS5_PORT|:::$SOCKS5_PORT)" >/dev/null 2>&1; then
            verification_passed=true
            info "✓ Port $SOCKS5_PORT listening (netstat command)"
            break
        fi
        
        # Method 4: Connection test
        if command -v nc >/dev/null 2>&1; then
            if timeout 3 nc -z 127.0.0.1 "$SOCKS5_PORT" 2>/dev/null || timeout 3 nc -z ::1 "$SOCKS5_PORT" 2>/dev/null; then
                verification_passed=true
        ...
        # Method 5: SOCKS5 functional test (if no auth)
        if [[ $SOCKS5_AUTH_ENABLED -eq 0 ]] && [[ $attempt -ge 3 ]]; then
            if timeout 10 curl --connect-timeout 3 --max-time 8 --socks5 127.0.0.1:$SOCKS5_PORT http://httpbin.org/ip 2>/dev/null | grep -q "origin"; then
                verification_passed=true
                info "✓ SOCKS5 functional test successful"
                break
            fi
        fi
    ...
    if [[ "$verification_passed" == true ]]; then
        log "SOCKS5 proxy service verification successful!"
        ...
        local port_info=""
        if command -v lsof >/dev/null 2>&1; then
            port_info=$(lsof -i ":$SOCKS5_PORT" 2>/dev/null | grep LISTEN || echo "")
        fi
        
        if [[ -n "$port_info" ]]; then
            echo "$port_info"
        else
            ss -tuln 2>/dev/null | grep ":$SOCKS5_PORT" || netstat -tuln 2>/dev/null | grep ":$SOCKS5_PORT" || echo "Port information not available via standard tools"
        fi
        
        # Show recent logs
        echo ""
        echo "Recent Service Logs:"
        journalctl -u "$SERVICE_NAME" --no-pager -n 5 | tail -5
        echo ""
        
        return 0
    else
        warning "Service verification failed after $max_attempts attempts"
        ...
        echo ""
        echo "Network Status:"
        ss -tuln 2>/dev/null | grep -E "(127\.0\.0\.1|0\.0\.0\.0|::)" | head -10
        echo ""
        
        # One final attempt with extended functional test
        if [[ $SOCKS5_AUTH_ENABLED -eq 0 ]]; then
            info "Final functional test attempt..."
            if timeout 15 curl --connect-timeout 5 --max-time 12 --socks5 127.0.0.1:$SOCKS5_PORT http://httpbin.org/ip 2>/dev/null | grep -q "origin"; then
                warning "Verification methods failed, but functional test passed!"
                warning "The service appears to be working despite detection issues."
                return 0
            fi
        fi
        
        error "Could not verify SOCKS5 service is working properly"
        echo ""
        echo "Troubleshooting suggestions:"
        echo "1. Manual test: cd $INSTALL_DIR && python3 socks5_server.py --bind 0.0.0.0:$SOCKS5_PORT"
...
complete_uninstall() {
    ...
    for config_dir in "/etc/socks5" "/opt/socks5" "/var/lib/socks5" "/var/log/socks5"; do
...
main() {
    ...
    if start_and_test_service; then
        ...
        echo "1. Check if service can be started manually:"
        echo "   cd $INSTALL_DIR && python3 socks5_server.py --bind 0.0.0.0:$SOCKS5_PORT"
        echo ""
        echo "2. Check service logs:"
        echo "   journalctl -u $SERVICE_NAME -f"
        echo ""
        echo "3. The service might still be working despite verification failure."
        echo "   Try testing with a SOCKS5 client to confirm."
        exit 1
    fi
}
...
case "${1:-install}" in
    ...
    --help|-h|help)
        ...
        echo ""
        echo "Commands:"
        echo "  install    Install SOCKS5 proxy (default)"
        echo "  uninstall  Completely remove SOCKS5 proxy"
