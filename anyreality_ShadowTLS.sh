...
# 完全卸载 sing-box，包括 service、配置、日志和二进制
full_uninstall() {
    log "开始完全卸载 sing-box..."
    # 停止服务
    if systemctl is-active --quiet sing-box; then
...
# 一键安装 ShadowTLS v3 模块
# 根据原作者 sd87671067 的脚本实现，提供简单的一键部署 ShadowTLS 的功能。
# 用户可以在顶层菜单中选择该安装方式。
install_shadowtls() {
    clear
    echo -e "${BLUE}sing-box ShadowTLS v3 自动安装脚本${NC}"
    echo -e "${YELLOW}根据 singbox-install 脚本改编，快速部署 ShadowTLS v3${NC}"
    echo "================================================"
    echo
    # 检查 root 权限
    if [[ $EUID -ne 0 ]]; then
    ...
    # 对于 ShadowTLS 默认使用 IPv4，因此无需检测 IPv6
    # 安装依赖
    install_dependencies

    # 如检测到旧的 sing-box 安装，先完整卸载以避免文件被占用
    if systemctl list-units --type=service --state=active | grep -q "sing-box.service"; then
        log "检测到已有 sing-box 服务，执行完整卸载..."
        full_uninstall
    elif [[ -f /usr/local/bin/sing-box ]]; then
        # 如果二进制文件存在但服务未运行，也做卸载
        log "检测到旧的 sing-box 二进制文件，执行完整卸载..."
        full_uninstall
    fi
    # 生成随机密码
    SHADOWTLS_PASSWORD=$(openssl rand -hex 16)
    SS_PASSWORD=$(openssl rand -base64 32)
    # 获取服务器 IP (强制使用 IPv4 模式)
    NETWORK_MODE="ipv4"
    get_server_ip
    # 读取端口与伪装域名，可回车使用默认值
    read -p "请输入监听端口 [443]: " ST_PORT
    ST_PORT=${ST_PORT:-443}
    read -p "请输入伪装域名 [www.bing.com]: " ST_SNI
    ST_SNI=${ST_SNI:-www.bing.com}
    # 创建 sing-box 配置目录
    mkdir -p /etc/sing-box
    mkdir -p /var/log/sing-box
    # 安装 sing-box 二进制，如果未安装或版本过低
    install_singbox
    # 生成 ShadowTLS 配置文件
    cat > /etc/sing-box/config.json << EOF
{
...
EOF
    log "ShadowTLS 配置文件已生成: /etc/sing-box/config.json"
    # 创建并启动服务
    create_systemd_service
    # 配置防火墙端口
    LISTEN_PORT=${ST_PORT}
    configure_firewall
    # 生成分享链接（Shadowsocks + ShadowTLS 插件）
    local ss_userinfo=$(echo -n "2022-blake3-chacha20-poly1305:${SS_PASSWORD}" | base64 -w0)
    local plugin_json="{\"version\":\"3\",\"host\":\"${ST_SNI}\",\"password\":\"${SHADOWTLS_PASSWORD}\"}"
    local plugin_base64=$(echo -n "$plugin_json" | base64 -w0)
    SHARE_LINK="ss://${ss_userinfo}@${SERVER_IP}:${ST_PORT}?shadow-tls=${plugin_base64}#ShadowTLS"
    # 显示和保存配置
    echo
    log "ShadowTLS 安装完成！"
    log "分享链接: $SHARE_LINK"
    # 保存配置文件和分享信息
    cat > /root/shadowtls_client_config.txt << EOF
=== sing-box ShadowTLS v3 客户端配置 ===

服务器信息:
- 服务器地址: ${SERVER_IP}
- 端口: ${ST_PORT}
- 伪装域名 (SNI): ${ST_SNI}
- Shadowsocks 方法: 2022-blake3-chacha20-poly1305
...
说明: 将链接导入支持 ShadowTLS v3 的客户端 (如 Shadowrocket)。
EOF
    log "客户端配置已保存到 /root/shadowtls_client_config.txt"
    # 生成二维码
    generate_qrcode "$SHARE_LINK" "/root/shadowtls_qrcode.png"
    # 显示完整客户端配置文件内容，便于手动配置
    if [[ -f /root/shadowtls_client_config.txt ]]; then
        echo
        echo -e "${CYAN}=== ShadowTLS 客户端完整配置内容 ===${NC}"
        cat /root/shadowtls_client_config.txt
        echo -e "${CYAN}================================================${NC}"
    fi
}
...
main_menu() {
    clear
    echo -e "${BLUE}sing-box 一键管理脚本${NC}"
    echo "==============================================="
    echo "请选择操作:"
    echo "1) 安装/重新安装 AnyReality (AnyTLS + Reality)"
    echo "2) 安装/重新安装 ShadowTLS v3"
    echo "3) 卸载 sing-box (删除 AnyReality/ShadowTLS 配置)"
    echo "4) 退出"
